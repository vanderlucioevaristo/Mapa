...
DEFAULT_HTML_OUTPUT = "calendario_v2.html"
...
def construir_linhas_calendario(
    ...
    for registro in registros:
        ...
        cor_evento = str(classificacao["cor"])

        linha = linhas.get(local)
        if linha is None:
            linha = {
                "local": local,
                "eventos": {},
    ...
    linhas_ordenadas = sorted(
        linhas.values(),
        key=lambda item: remover_acentos(str(item["local"])).upper(),
    )
...
def montar_bloco_mes(ano: int, mes: int, eventos_mes: dict[int, list[dict[str, object]]]) -> str:
    total_dias = calendar.monthrange(ano, mes)[1]
    dias_html: list[str] = []

    # Quando o evento atravessa o mes, mostramos o rotulo no primeiro dia visivel.
    primeiro_dia_visivel_por_evento: dict[str, int] = {}
    for dia, eventos_dia in eventos_mes.items():
        for evento in eventos_dia:
            assinatura = "|".join(
                [
                    str(evento.get("descricao") or ""),
                    str(evento.get("data_inicio") or ""),
                    str(evento.get("data_final") or ""),
                    str(evento.get("endereco") or ""),
                    str(evento.get("publico") or ""),
                ]
            )
            if assinatura not in primeiro_dia_visivel_por_evento:
                primeiro_dia_visivel_por_evento[assinatura] = dia
    ...
    for dia in range(1, total_dias + 1):
        ...
        cor_celula = str(eventos_dia[0].get("cor") or "#64748b")
        ...
        tags = []
        for evento in eventos_dia:
            cor_evento = str(evento.get("cor") or "#64748b")
            ...
            exibir_rotulo = bool(evento.get("inicio_periodo")) or (
                primeiro_dia_visivel_por_evento.get(assinatura) == dia
            )
            if not exibir_rotulo:
                continue

            tags.append(
                f'<div class="event-tag" style="--event-color:{cor_evento}" title="{descricao} | Periodo: {data_inicio} ate {data_final} | Publico: {publico} | Endereco: {endereco}">{descricao}</div>'
            )

        dias_html.append(
            f'<div class="{" ".join(classes)}" style="--event-color:{cor_celula}"><span class="day-number">{dia}</span><div class="event-details">{"".join(tags)}</div><div class="event-count">{len(eventos_dia)} evento(s)</div></div>'
        )
...
def montar_html(
    ...
    for ano in anos:
        ...
        for linha in linhas:
            ...
            celulas_mes = "".join(
                f"<td>{montar_bloco_mes(ano, mes, por_mes[mes])}</td>"
            ...
            linhas_tabela.append(
                "<tr>"
                f"<th class=\"local-cell\"><div class=\"local-name\">{local}</div></th>"
                f"{celulas_mes}"
                "</tr>"
            )
...
<head>
  ...
  <title>Calendario de Eventos por Local — Visite Belo Horizonte</title>
...
<body>
  <main class="shell">
    <section class="hero">
      ...
      <h1>Calendario de Eventos por Local</h1>
    <p>Linhas agrupadas somente por local. O porte do evento e identificado pela cor na grade do calendario.</p>
